...
async function fetchHtmlPlain(url, extraHeaders) {
  const headers = {
...
async function fetchHtmlPlaywright(url, extraHeaders) {
...
async function fetchHtmlJina(url, extraHeaders) {
...
async function getHtml(url, opts = {}) {
  ...
  const autoJina = Boolean(opts.autoJina);
  const autoPlaywright = Boolean(opts.autoPlaywright);

  let plain = null;
  let plainError = null;
  const extraHeaders = opts.headers || null;
  try {
    plain = await fetchHtmlPlain(url, extraHeaders);
  } catch (err) {
    plainError = err;
  }
  ...
  if (useJina || autoJina) {
    const rendered = await fetchHtmlJina(url, extraHeaders);
    setCached(url, rendered.text);
    return rendered.text;
  }

  if (usePlaywright || autoPlaywright) {
    const rendered = await fetchHtmlPlaywright(url, extraHeaders);
    setCached(url, rendered.text);
    return rendered.text;
  }
